...
const targetDir = path.join(__dirname, '..', 'app', 'blog', '[slug]');
if (!fs.existsSync(targetDir)) {
  fs.mkdirSync(targetDir, { recursive: true });
}

// 1. ArticleSocialShare.tsx
const socialShareCode = `'use client';
...
`;
fs.writeFileSync(path.join(targetDir, 'ArticleSocialShare.tsx'), socialShareCode, 'utf8');

// 2. ArticleFaqAccordion.tsx
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 shrink-0 transition-transform duration-200 ${
                    isOpen ? 'rotate-180 text-sage' : 'text-muted'
                  }`}
                />
              </button>
              {isOpen && (
...
`;
fs.writeFileSync(path.join(targetDir, 'ArticleFaqAccordion.tsx'), faqAccordionCode, 'utf8');
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="rounded-3xl p-8 sm:p-10 bg-sage-pal border-2 border-sage/40 text-center space-y-5 shadow-sm mb-16">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
              <Link
                href="/free-audit"
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-sage text-white text-xs font-bold hover:opacity-90 transition-opacity shadow-xs text-center"
              >
                Claim Free Website SEO Audit →
              </Link>
              <Link
                href="/#contact"
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-card border border-border text-ink text-xs font-bold hover:bg-cardSubtle transition-colors shadow-xs text-center"
              >
          ...
          <div className="mt-12 text-center">
            <Link
              href="/blog"
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl border border-border text-xs font-semibold text-muted hover:text-ink hover:bg-cardSubtle transition-colors shadow-2xs"
            >
...
console.log('Created blog slug files successfully!');